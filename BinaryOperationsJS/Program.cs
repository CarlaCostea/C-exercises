...
namespace BinaryOperations
{
    class Program
    {
        ...
        const int OperationFactorial = 17;

        static void Main()
        {
            if (int.TryParse(Console.ReadLine(), out int operation) && operation >= OperationConvertToBinary && operation <= OperationFactorial)
            {
        ...
        private static void ExecuteArithmeticalOperation(int operation)
        {
            ...
            else if (operation >= OperationBinaryAdd && operation <= OperationBinaryMultiply)
            {
                BaseBinaryOperation(operation);
            }
            else
            {
                FactorialOperation(operation);
            }
        }

        private static void FactorialOperation(int operation)
        {
            ...
            if (operation != OperationFactorial)
            {
                return;
            }

            operation = 0;
            byte[] factorial = Factorial(binaryNumber);
            Console.WriteLine(BinaryNumberToString(factorial));
        }

        private static byte[] Factorial(byte[] binaryNumber)
        {
            if (GetBinaryNumberRealLength(binaryNumber) == 1)
            {
                return binaryNumber;
            }

            return BinaryMultiply(binaryNumber, Factorial(BinarySubtract(binaryNumber, new byte[] { 1 })));
        }
