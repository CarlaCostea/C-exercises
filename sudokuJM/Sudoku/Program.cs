    ...
    class Program
    {
        ...
        static bool IsValidSudokuItem(byte[,] sudokuBoard, string itemType, int itemIndex)
        {
            ...
            for (int i = 0; i < SudokuBoardSize; i++)
            {
                ...
                if (sudokuValuesCount[sudokuValue - 1] > 1)
                {
                    if (itemType == "line")
                    {
                        Console.WriteLine("Elementul " + sudokuValue + " apare de mai multe ori pe linia " + (itemIndex + 1));
                    }

                    if (itemType == "column")
                    {
                        Console.WriteLine("Elementul " + sudokuValue + " apare de mai multe ori pe coloana " + (itemIndex + 1));
                    }

                    if (itemType == "block")
                    {
                        Console.WriteLine("Elementul " + sudokuValue + " apare de mai multe ori in blocul " + (itemIndex + 1));
                    }
        ...
        static bool ReadSudokuBoard(byte[,] sudokuBoard)
        {
            for (int i = 0; i < SudokuBoardSize; i++)
            {
                ...
                {
                    Console.WriteLine(lineValues.Length > SudokuBoardSize ? "Sunt mai mult de 9 elemente pe linia " + (i + 1) : "Sunt sub 9 elemente pe linia " + (i + 1));
                ...
                for (int j = 0; j < SudokuBoardSize; j++)
                {
                    ...
                    {
                        Console.WriteLine("Element invalid pe linia " + (i + 1) + ": " + lineValues[j]);
                        return false;
                    }
